...
const CategoryCard = ({ cat }: { cat: Category }) => {
  // safe properties (adjust if your Category shape differs)
  ...
  return (
    <Link href={`/category/${slug}`} key={slug}>
      <a className="block w-44 sm:w-52 p-3 rounded-2xl border hover:shadow-md transition bg-white">
...
const CategoriesListCarousel = ({ categories }: { categories: Category[] }) => {
  // Defensive: if categories is undefined, show placeholder
  ...
  // Build item nodes for Carousel (if your Carousel accepts nodes)
  ...
  return (
    <div className="py-6">
      ...
      {/* Preferred: use project's Carousel component if available */}
      {Carousel ? (
        <div className="px-4">
          <Carousel Items={items} swiper_slide_style="!w-[200px] !h-auto !px-2" />
        </div>
      ) : (
        // Fallback: simple horizontal scroll (mobile-first)
